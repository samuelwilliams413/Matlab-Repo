...
for trial = 1:1:1             %for all speed limits
    for g = 1:1:1   %for all gear ratios
        ...
        for inc = 1:1:length(d_int)          %every 10 m
            ...
            vc(trial,g,inc) = LIMITS_gov(index);
        end
    end
end
...
for trial = 1:1:trials             %for all speed limits
    ...
    for g = 1:1:gears   %for all gear ratios
        ...
        for inc = 1:1:length(d_int)          %every 10 m
            ...
            f_store(inc,1) = get_F_inertia(a);
            f_store(inc,2) = get_F_grade(m);
            f_store(inc,3) = get_F_aero(v);
            f_store(inc,4) = get_F_roll(g);
